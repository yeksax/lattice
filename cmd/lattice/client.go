...
func cliShare(slug string, random bool) error {
	...
	fmt.Println("only this summary and its /submit endpoint are public; results: lattice results " + slug)
	return nil
}
...
func cliShares() error {
	// Hosted shares first (if logged in), then local expose shares. Both
	// sections are shown so a mixed setup is legible at a glance.
	loggedIn := loadConfigClient().Hosted.Token != ""
	if loggedIn {
		fmt.Println("hosted:")
		if err := hostedSharesList(); err != nil {
			fmt.Printf("  (%v)\n", err)
		}
	}
	...
	if err != nil {
		if loggedIn {
			return nil // hosted section already printed; daemon just isn't up
		}
		return fmt.Errorf("server not running at %s", baseURL())
	}
	defer resp.Body.Close()
	...
	for _, sh := range shares {
		fmt.Printf("%-32s  %-40s  port %-5d  %d vote(s)\n", sh.Slug, sh.URL, sh.Port, sh.Votes)
	}
	return nil
}
