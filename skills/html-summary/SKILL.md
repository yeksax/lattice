...
description: >-
  ...
  pages, and shareable single-file docs in Lattice's own visual language — monospace,
...
one `<style>`, no external requests, no build step) that the user shares with
...
The user can customize the look from the lattice desktop app. That writes
`~/.summaries/.lattice/config.json`. **Before writing any CSS, read that file**
(it may not exist). If it has a non-empty `theme`, it is the user's *deliberate*
...
`font`, honor it — those bans describe the default, not a choice the user explicitly
configured.
...
Lattice favors small interactions, but **only where a static frame is genuinely worse.**
...
Bars are Lattice's **stacked segmented bar**: one composite bar, ~7–10px tall, on a
...
If the user shares it externally, offer `expose <port> <sub>` to serve it (see the
global expose rule) or just hand them the file.

## Register in lattice (the summary knowledge base)

Every summary gets registered in **lattice** (`~/dev/personal/lattice`), the local
app serving `~/.summaries` at `http://127.0.0.1:4600`.
...
(`lattice share <slug>` → `https://<slug>.example.dev`). Lattice injects a poll
